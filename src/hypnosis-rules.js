...
export const DEFAULT_HYPNOSIS_RULESET_VERSION = '4.3.0-hypnoos.6';
...
const CORE_RULES = Object.freeze([
  ...
  '除本轮有效催眠操作或既有效果文本明确修改记忆外，不得默认让目标失忆、断片、记忆模糊或自动合理化。常识修改成功时，目标只在该效果的对象、内容、时效和范围内把新常识视为自然，并以当前常识行动；不得外推到未修改事项。临时常识到期后不再强制，永久常识只有明确解除才消失。',
  ...
  '用户输入的行动只是尝试而非既定成功。普通行动须遵守现实逻辑、人物能力、地点、时间和既有关系；角色不得为推进剧情而降智。高智力角色始终保留批判性思维，催眠只改变命令明确覆盖的意愿、感知、记忆、身体或常识范围。有效催眠操作在权限、余额、目标和剧情条件成立时应正常成功，不得用“更困难”无故制造失败。',
  ...
  '变量权限中的通配根只是最大能力包络，必须被每条中文指令、效果时效、作用域和唯一结果位置收窄。AI只写本轮操作明确允许的精确路径；AI不动和前端已写路径不得重复修改。相关变量只是验算快照，不是写入许可。',

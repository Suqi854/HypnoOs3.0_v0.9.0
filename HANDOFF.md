...
- 最新远端提交以 `git ls-remote origin refs/heads/main` 为准；本轮用户已明确要求修复后推送。
- 当前关键提交：
...
- 核心模块与手机前端均带 `database-profile-v3` 缓存版本标识，避免浏览器继续执行修复前的数据库适配模块。
...
- UI 基线：`f00a23a76ce0a4c525e33812f538068b955f809cf51e631efb9c93ce1aa2b7e5`（仅数据库档案读取逻辑变化，布局与样式未改）。

(function () {
  ...
  function createRegistry(host) {
    ...
    var requestFrame = host.requestAnimationFrame
      ? host.requestAnimationFrame.bind(host)
      : function (callback) { return host.setTimeout(function () { callback(Date.now()); }, 16); };
    ...
    function scheduleActionFoldRender() {
      ...
      actionFoldRenderFrame = requestFrame(function () {
    ...
    function syncPanelSize(requestedScale) {
      ...
      var preferred = normalizedPhoneScale(requestedScale === undefined ? readUiState().scale : requestedScale);
    ...
    function advancePetFrame(timestamp) {
      petMotionFrame = 0;
      if (!petSprite || !launcher) return;
      var meta = petStateMeta(petState);
      var now = Number(timestamp) || Date.now();
      var interval = 1000 / Math.min(16, Math.max(1, meta.fps));
      if (!petMotionLast) petMotionLast = now;
      if (now - petMotionLast < interval) {
        petMotionFrame = requestFrame(advancePetFrame);
        return;
      }
      petMotionLast = now - ((now - petMotionLast) % interval);
      petFrame += 1;
    ...
    function shellCss() {
      return [
        ...
        "@keyframes pet-miku-click{0%,100%{transform:none}32%{transform:translate3d(0,-13px,0) rotate(-6deg) scale(1.05,.95)}62%{transform:translate3d(0,-2px,0) rotate(7deg) scale(.97,1.04)}}@keyframes pet-miku-long{0%,100%{transform:none}25%{transform:translate3d(-7px,-2px,0) rotate(-8deg)}50%{transform:translate3d(7px,-4px,0) rotate(8deg)}75%{transform:translate3d(-4px,-2px,0) rotate(-5deg)}}@keyframes pet-miku-drag{0%,100%{transform:translate3d(-3px,-5px,0) rotate(-5deg)}50%{transform:translate3d(3px,2px,0) rotate(5deg)}}.launcher[data-pet-character='miku'][data-pet-state='unique_a'] .pet-visual{animation:pet-miku-click .74s cubic-bezier(.2,.82,.25,1)}.launcher[data-pet-character='miku'][data-pet-state='unique_b'] .pet-visual{animation:pet-miku-long .9s ease-in-out}.launcher[data-pet-character='miku'][data-pet-state='held_scared'] .pet-visual{animation:pet-miku-drag .42s ease-in-out infinite}@keyframes pet-rem-click{0%,100%{transform:none}34%{transform:translate3d(0,3px,0) rotate(5deg) scale(1.03,.94)}66%{transform:translate3d(0,-4px,0) rotate(-3deg) scale(.98,1.03)}}@keyframes pet-rem-long{0%,100%{transform:none}30%{transform:translate3d(-4px,2px,0) rotate(-6deg) scale(1.04,.92)}60%{transform:translate3d(3px,-2px,0) rotate(4deg) scale(.98,1.04)}}@keyframes pet-rem-drag{0%,100%{transform:translate3d(-2px,-3px,0) rotate(-2deg)}25%{transform:translate3d(2px,1px,0) rotate(3deg)}50%{transform:translate3d(-1px,-1px,0) rotate(-3deg)}75%{transform:translate3d(2px,2px,0) rotate(2deg)}}.launcher[data-pet-character='rem'][data-pet-state='unique_a'] .pet-visual{animation:pet-rem-click .7s ease-out}.launcher[data-pet-character='rem'][data-pet-state='unique_b'] .pet-visual{animation:pet-rem-long .86s ease-in-out}.launcher[data-pet-character='rem'][data-pet-state='held_scared'] .pet-visual{animation:pet-rem-drag .24s linear infinite}@keyframes pet-mai-click{0%,100%{transform:none}30%{transform:translate3d(7px,-4px,0) rotate(5deg)}64%{transform:translate3d(-3px,1px,0) rotate(-3deg)}}@keyframes pet-mai-long{0%,100%{transform:none}20%{transform:translate3d(0,-2px,0) rotate(-2deg)}35%{transform:translate3d(0,1px,0) rotate(2deg)}50%{transform:translate3d(0,-2px,0) rotate(-2deg)}65%{transform:translate3d(0,1px,0) rotate(2deg)}}@keyframes pet-mai-drag{0%,100%{transform:translate3d(-2px,-4px,0) rotate(-7deg)}50%{transform:translate3d(2px,1px,0) rotate(-4deg)}}.launcher[data-pet-character='mai'][data-pet-state='unique_a'] .pet-visual{animation:pet-mai-click .68s cubic-bezier(.2,.78,.3,1)}.launcher[data-pet-character='mai'][data-pet-state='unique_b'] .pet-visual{animation:pet-mai-long .8s ease-in-out}.launcher[data-pet-character='mai'][data-pet-state='held_scared'] .pet-visual{animation:pet-mai-drag .36s ease-in-out infinite}@keyframes pet-umaru-click{0%,100%{transform:none}22%{transform:translate3d(0,-11px,0) scale(1.08,.9)}45%{transform:translate3d(0,2px,0) scale(.93,1.08)}67%{transform:translate3d(0,-7px,0) scale(1.05,.94)}}@keyframes pet-umaru-long{0%,100%{transform:none}35%{transform:translate3d(0,5px,0) scale(1.12,.82)}70%{transform:translate3d(0,2px,0) rotate(5deg) scale(1.06,.9)}}@keyframes pet-umaru-drag{0%,100%{transform:translate3d(-4px,-2px,0) rotate(-6deg)}25%{transform:translate3d(4px,2px,0) rotate(6deg)}50%{transform:translate3d(-3px,1px,0) rotate(-5deg)}75%{transform:translate3d(3px,-1px,0) rotate(5deg)}}.launcher[data-pet-character='umaru'][data-pet-state='unique_a'] .pet-visual{animation:pet-umaru-click .76s cubic-bezier(.2,.85,.25,1)}.launcher[data-pet-character='umaru'][data-pet-state='unique_b'] .pet-visual{animation:pet-umaru-long .9s ease-in-out}.launcher[data-pet-character='umaru'][data-pet-state='held_scared'] .pet-visual{animation:pet-umaru-drag .22s linear infinite}",
...
".pet-menu{position:fixed;z-index:4;width:120px;height:56px;pointer-events:none;opacity:0;transform:translateY(8px) scale(.9);transform-origin:50% 100%;transition:opacity .14s ease,transform .14s ease}.pet-menu.open{pointer-events:auto;opacity:1;transform:none}.pet-menu button{position:absolute;top:0;width:56px;height:56px;padding:0;border:1px solid rgba(226,232,240,.7);border-radius:50%;background:linear-gradient(145deg,rgba(30,41,59,.97),rgba(15,23,42,.98));box-shadow:0 8px 22px rgba(2,6,23,.48),inset 0 1px rgba(255,255,255,.14);color:#f8fafc;font:850 11px/1.1 system-ui;cursor:pointer;touch-action:manipulation}.pet-menu button[data-pet-action='choose']{left:0}.pet-menu button[data-pet-action='settings']{left:64px}.pet-menu button:hover,.pet-menu button:focus-visible{border-color:#a5f3fc;background:linear-gradient(145deg,#155e75,#172554);outline:2px solid rgba(165,243,252,.72);outline-offset:2px}",
        ...
        ".panel{pointer-events:auto;position:fixed;width:430px;height:812px;border:0;border-radius:38px;background:transparent;box-shadow:none;overflow:visible;z-index:5;display:none;isolation:isolate;--floor-sidecar-width:270px;--phone-scale:1;transform:translateZ(0) scale(var(--phone-scale));transform-origin:0 0;backface-visibility:hidden;will-change:transform}",
        ".panel.open{display:block}",
".phone-wrap{position:absolute;z-index:4;left:0;top:0;width:430px;height:812px;border:1px solid rgba(221,184,255,.3);border-radius:inherit;background:transparent;box-shadow:0 20px 60px rgba(0,0,0,.42);overflow:hidden;isolation:isolate;contain:layout paint;backface-visibility:hidden}.phone-wrap:after{content:'';position:absolute;inset:0;border-radius:inherit;box-shadow:inset 0 0 0 1px rgba(255,255,255,.07);pointer-events:none;z-index:8}.phone{display:block;width:430px;height:812px;border:0;background:transparent}",
    ...
    function bridgePrelude() {
      ...
      return "<script>(function(){var r=parent[" + singletonKey + "];window.__ST_HYPNOOS_FLOATING_PHONE__=true;window.__ST_HYPNOOS_FLOATING_REGISTRY__=r;window.__ST_HYPNOOS_ASSET_BASE__=" + asset + ";" +
        ...
        "['eventOn','triggerSlash','getWorldbookNames','getCharWorldbookNames','getWorldbook','generateRaw','getArchiveWorldbookOptions','configureArchiveWorldbook','syncArchiveFromLatestReply','getDatabaseSnapshot','syncDatabaseState'].forEach(function(n){if(r.hasApi(n))globalThis[n]=function(){return r.callApi(n,Array.prototype.slice.call(arguments))}});" +
...
"<nav class='pet-menu' role='menu' aria-label='桌宠菜单' aria-hidden='true'><button type='button' role='menuitem' data-pet-action='choose'>选择</button><button type='button' role='menuitem' data-pet-action='settings'>通用</button></nav>" +
    ...
    function flushResizeFrame() {
      ...
      var horizontal = (point.x - resizeState.startX) * (resizeState.corner === "left" ? -1 : 1);
      var vertical = point.y - resizeState.startY;
      var delta = (horizontal * 430 + vertical * 812) / (430 * 430 + 812 * 812);
      var scale = syncPanelSize(resizeState.startScale + delta);
      var size = { width: Math.max(1, 430 * scale), height: Math.max(1, 812 * scale) };
